...
class DataModule(Dataset):
    ...
    def __getitem__(self, index):
        ...
        source_string = f'<x> {source} </x> is <y>' ## TODO
        target_string = f'<x> {source} </x> is <y> {target} </y>.' ## TODO
        source_tensor = self.convert_sentence_to_input(source_string, self.config.max_source_length)
        target_tensor = self.convert_sentence_to_input(target_string, self.config.max_target_length)
