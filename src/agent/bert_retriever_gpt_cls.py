...
class Agent():
    def __init__(self,
        ...
        ## set tokenizer
        self.tokenizer = {
                'retriever': self.set_tokenizer(self.config.tokenizer.retriever),
                'generator': self.set_tokenizer(self.config.tokenizer.generator)
                }
        ## set data, labels, and knowledge base
        self.set_data(self.config.mode)
        ## set model
        if self.config.model.model.retriever.num_kbs == -1:
            self.config.model.model.retriever.num_kbs = len(self.knowledge_base)
        self.model = self.set_model(self.config.model.model)
        self.optimizer = self.set_optimizer(self.config.model.optimizer)
        ## set retriever knowledge base
        self.model.retriever.set_kb_embeddings(self.knowledge_base)
    ...
    def set_model(self, config):
        ...
        model = hydra.utils.instantiate(config,
                ...
                max_source_length=self.config.datamodule.max_source_length,
                max_target_length=self.config.datamodule.max_target_length,
                max_kb_length=self.config.datamodule.max_kb_length,
                )
    ...
    def set_data(self, mode):
        ...
        def get_train_dataloader():
            return self.set_dataloader(self.config.datamodule, tokenizer=self.tokenizer['retriever'], 
        ...
        self.labels = dataset.get_labels() ## for classification
        self.knowledge_base = dataset.get_kb( data_path=os.path.join( self.config.work_dir,
                                                                      self.config.datamodule.data_dir,
                                                                      self.config.datamodule.kb_data)
                                            )
    ...
    def predict(self, dir_path=None):
        self.model.eval()
        if not dir_path:
            dir_path = self.config.model.model.retriever.path
