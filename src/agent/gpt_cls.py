...
class Agent():
    ...
    def fit(self): ## TRAINING
        ...
        for epoch in range(self.config.agent.epochs):
            ...
            for index, item in enumerate(dataloader): ## 1 epoch
                batch = {'input_ids':item['labels'], 'labels':item['labels']}
    ...
    def predict(self, dir_path=None):
        ...
        for index, item in enumerate(dataloader): ## 1 epoch
            ...
            #output = self.model.predict_step(batch, index, max_length=self.config.datamodule.max_target_length)
            output = self.model.predict_step(batch, index, max_length=self.config.datamodule.max_target_length*2)
            ...
            for origin_data, model_output in zip(item['data'], output):
                ...
                for key in ['preds', 'labels']:
                    ...
                    if '<y>' in decoded_output: decoded_output = decoded_output[decoded_output.index('<y>'):]
                    decoded_output = decoded_output.replace('<y>','').strip()
                    if '</y>' in decoded_output: decoded_output = decoded_output[:decoded_output.index('</y>')]
                    decoded_output = decoded_output.strip()
                    model_output[key] = decoded_output
